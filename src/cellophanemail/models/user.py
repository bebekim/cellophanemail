...
from piccolo.columns import Varchar, Text, Boolean, Timestamp, ForeignKey, UUID, JSON
...
class User(Table, tablename="users"):
    ...
    phone_number = Varchar(length=20, unique=True, index=True, null=True)
    ...
    verification_method = Varchar(length=20, null=True)  # "email" or "phone"
    ...
    subscription_status = Varchar(
        length=50, default=SubscriptionStatus.FREE, choices=SubscriptionStatus
    )
    ...
    def __str__(self):
        identifier = self.email or self.phone_number or "no-identifier"
        return f"User({identifier}, active={self.is_active})"
