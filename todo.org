...
** TODO Railway Staging Environment Configuration
   SCHEDULED: <2026-02-02>
   - [ ] Configure Railway dashboard: test environment → watch `staging` branch
         Go to: https://railway.com/project/77cfc010-27b0-4265-90a9-6a90251d7459
         1. Select `test` environment
         2. Click `cellophanemail` service → Settings → Source
         3. Change branch from `main` to `staging`

   **Deployment Workflow After Configuration:**
   - `git push origin staging` → deploys to test (cellophanemail-test.up.railway.app)
   - `git push origin main` → deploys to production (cellophanemail-production.up.railway.app)

** TODO Implement Forgot Password Flow
   SCHEDULED: <2026-02-02>
   - [ ] Create `/auth/forgot-password` route in FrontendController
   - [ ] Create forgot password template (auth/forgot_password.html)
   ...
   - [ ] Create reset password template (auth/reset_password.html)
...
** TODO Docker Environment Validation Tests
   - [ ] Create tests/docker/test_docker_environment.py
   - [ ] Add test_docker_compose_config_valid() - Validate docker-compose.yml syntax
   - [ ] Add test_dockerfile_builds() - Verify Dockerfile builds without errors
   - [ ] Add test_services_start() - All services (db, app, smtp) start successfully
   - [ ] Add test_health_endpoints() - Health check endpoints respond correctly
   - [ ] Add test_hot_reload_works() - Code changes trigger automatic reload
   - [ ] Add test_database_persistence() - Database data persists across container restarts
   - [ ] Configure CI/CD to run Docker tests (requires Docker in CI environment)

   **Context:** Core Docker implementation complete and merged to main.
   These tests would provide automated regression detection for Docker setup.
   See: thoughts/shared/plans/2025-10-11-docker-development-environment.md:954-1069
